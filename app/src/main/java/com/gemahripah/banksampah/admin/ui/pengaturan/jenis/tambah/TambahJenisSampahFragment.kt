...
class TambahJenisSampahFragment : Fragment(R.layout.fragment_tambah_jenis_sampah) {
            ...
            try {
                val result = withContext(Dispatchers.IO) {
                    supabase.postgrest["kategori"]
                        .select(columns = Columns.list("id", "created_at", "nama"))
                        .decodeList<Kategori>()
                }

                listKategori = result

                val namaList = result.map { it.nama }
                ...
                kategoriView.setOnItemClickListener { _, _, position, _ ->
                    ...
                    val selectedKategori = listKategori.find { it.nama == selectedNama }
    ...
    private fun setupKonfirmasiButton() {
        binding.konfirmasi.setOnClickListener {
            ...
            val harga = binding.harga.text.toString().trim().toLongOrNull()
            val keterangan = binding.keterangan.text.toString().trim()

            if (selectedKategoriId == null) {
                Toast.makeText(requireContext(), "Silakan pilih kategori sampah", Toast.LENGTH_SHORT).show()
