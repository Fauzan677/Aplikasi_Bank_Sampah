package com.gemahripah.banksampah.admin.ui.pengaturan.adapter
...
class KategoriAdapter(
    ...
    inner class KategoriViewHolder(private val binding: ItemJenisSampahBinding) :
        ...
        fun bind(kategori: Kategori) {
            binding.kategori.text = kategori.nama

            CoroutineScope(Dispatchers.Main).launch {
                try {
                    val jenisList = SupabaseProvider.client
                        .from("sampah")
                        .select(columns = Columns.list("id, created_at, kategori, jenis, satuan, harga, keterangan")) {
                            filter {
                                eq("kategori", kategori.id ?: 0)
                            }
                        }
                        .decodeList<Sampah>()
                    ...
                    val jenisWithKategori = jenisList.map {
                        KategoridanSampah(it, kategori.nama)
                    }
